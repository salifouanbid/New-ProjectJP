...
async function ensureSuperadmin() {
  const exists = await db.maybeOne("SELECT 1 FROM users WHERE role = 'superadmin' LIMIT 1");
  if (exists) return;
  ...
  await db.execute(
    `INSERT INTO users (school_id, username, password_hash, role, first_name, last_name, must_change_password)
     VALUES (NULL, $1, $2, 'superadmin', 'Propriétaire', 'Plateforme', $3)`,
    [config.superadmin.username, hash, generated]
  );
  console.log('\n==============================================================');
