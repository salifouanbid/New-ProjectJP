...
async function load(req, res, allowPasswordChange) {
  ...
  const user = await db.maybeOne('SELECT * FROM users WHERE id = $1', [payload.uid]);
  if (!user || !user.active) { res.status(401).json({ error: 'Compte inactif', code: 'AUTH' }); return false; }
  let school = null;
  if (user.school_id) {
    school = await db.maybeOne('SELECT * FROM schools WHERE id = $1', [user.school_id]);
...
const authMiddleware = (allowPasswordChange) => (req, res, next) => {
  load(req, res, allowPasswordChange).then((ok) => { if (ok) next(); }).catch(next);
};
const authenticate = authMiddleware(false);
const authenticateAllowPasswordChange = authMiddleware(true);
